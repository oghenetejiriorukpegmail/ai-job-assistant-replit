...
const { connectDB, isUsingReplitDB, isUsingMongoDB, isUsingMemory } = require('./config/database');
...
app.get('/api/health', (req, res) => {
  res.status(200).json({
    ...
    databaseMode: isUsingReplitDB() ? 'replit-database' : 
                  isUsingMongoDB() ? 'mongodb' :
                  'in-memory-fallback',
    environment: process.env.REPL_ID ? 'replit' : 'local',
    warning: isUsingMemory() ? 'Using in-memory storage. Data will not persist after server restart.' : null
  });
});
